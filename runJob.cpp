...
void runJob::Run(){
    ...
    if(myid == 0){
        ...
        while(aim == 0) aim = rand() % Num_p;
        cout << "The first machine is " << aim <<  endl;
        MPI_Send(&buf,MAX_Q_V + MAX_Q_E,MPI_INT,aim,DATATAG,MPI_COMM_WORLD);
    }
    while(1){
